...
if (!function_exists('post_card_posted_by')) {
    ...
    function post_card_posted_by() {
        printf(
                ...
                '<span class="byauthor">%1$s<span class="screen-reader-text">%2$s</span><span class="author vcard"><a class="url fn n" href="%3$s">%4$s</a></span></span>', '<i class="fas fa-user" aria-hidden="true"></i>', esc_html__('Posted by', CEE_DOMAIN), esc_url(get_author_posts_url(get_the_author_meta('ID'))), esc_html(get_the_author())
        );
    }
...
if (!function_exists('post_card_2_posted_by')) {
    ...
    function post_card_2_posted_by() {
        printf(
                /* translators: 1: SVG icon. 2: post author, only visible to screen readers. 3: author link. */
                '<span class="byauthor">%1$s<span class="screen-reader-text">%2$s</span><span class="author vcard"><a class="url fn n" href="%3$s">%4$s</a></span></span>', __(get_avatar(get_the_author_meta('ID'), 40)), esc_html__('By', CEE_DOMAIN), esc_url(get_author_posts_url(get_the_author_meta('ID'))), esc_html(get_the_author())
        );
    }
    ...
    function post_card_posted_categories() {
        ...
        if ($categories_list) {
            printf(
                    ...
                    '<span class="cat-links">%1$s<span class="screen-reader-text">%2$s</span>%3$s</span>', '', esc_html__('Posted in', CEE_DOMAIN), $categories_list
            ); // WPCS: XSS OK.
        }
    }
    ...
    function post_card_posted_tag() {
        ...
        if ($tags_list) {
            printf(
                    ...
                    '<div class="post-card-tags"><span class="tags-links">%1$s<span class="screen-reader-text">%2$s </span>%3$s</span> </div>', '<i class="fa fa-tags" aria-hidden="true"></i>', esc_html__('Tags:', CEE_DOMAIN), $tags_list
            ); // WPCS: XSS OK.
        }
    }
